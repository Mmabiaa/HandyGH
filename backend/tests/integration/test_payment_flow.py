...
from apps.payments.services import MoMoService, PaymentService
...
class TestPaymentBookingIntegration:
    ...
    def test_complete_payment_flow_success(self, customer_user, sample_provider, sample_booking):
        ...
        txn = PaymentService.initiate_payment(
            booking=sample_booking, payment_provider="MOMO", metadata={"phone": "0241234567"}
        )
    ...
    def test_payment_flow_with_failure(self, customer_user, sample_provider, sample_booking):
        """Test payment flow when payment fails."""
        # Initiate payment
        txn = PaymentService.initiate_payment(booking=sample_booking, payment_provider="MOMO")
    ...
    def test_payment_prevents_double_payment(self, sample_booking):
        ...
        # First payment
        txn1 = PaymentService.initiate_payment(booking=sample_booking, payment_provider="MOMO")

        PaymentService.process_payment_success(transaction_id=txn1.id, provider_ref="MOMO-123")
        ...
        with pytest.raises(ConflictError, match="already paid"):
            PaymentService.initiate_payment(booking=sample_booking, payment_provider="MOMO")

    def test_payment_with_commission_calculation(self, sample_booking, sample_provider):
    ...
    def test_webhook_idempotency_prevents_duplicate_processing(self, sample_booking):
        ...
        PaymentService.process_payment_success(transaction_id=txn.id, provider_ref="MOMO-123")
    ...
    def test_manual_payment_confirmation_flow(self, customer_user, sample_booking):
    ...
    def test_multiple_transactions_for_same_booking(self, sample_booking):
        ...
        txn2 = PaymentService.initiate_payment(booking=sample_booking, payment_provider="MOMO")

        assert txn1.id == txn2.id

        # Mark first as failed
        PaymentService.process_payment_failure(transaction_id=txn1.id, reason="Network error")

        # Now can create new transaction
        txn3 = PaymentService.initiate_payment(booking=sample_booking, payment_provider="MOMO")
