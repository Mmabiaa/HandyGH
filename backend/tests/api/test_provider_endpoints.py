...
class TestServiceCategoryEndpoints:
    ...
    def test_list_categories_excludes_inactive(self, api_client, plumbing_category):
        ...
        ServiceCategory.objects.create(name="Inactive", slug="inactive", is_active=False)
...
class TestCreateProviderEndpoint:
    ...
    def test_create_provider_duplicate(self, provider_client, plumbing_category):
        ...
        Provider.objects.create(user=provider_client.user, business_name="Existing Provider")
    ...
    def test_create_provider_invalid_category(self, provider_client):
        ...
        url = reverse("providers:provider-list")
        data = {"business_name": "Test", "categories": ["invalid_category"]}
...
class TestSearchProvidersEndpoint:
    ...
    def test_search_providers_by_location(self, api_client, provider_with_profile):
        ...
        url = reverse("providers:provider-list")
        params = {"latitude": "5.6037", "longitude": "-0.1870", "radius_km": "10"}
    ...
    def test_search_providers_sort_by_rating(self, api_client):
        ...
        Provider.objects.create(
            user=user1, business_name="High Rating", rating_avg=Decimal("4.8"), is_active=True
        )

        user2 = User.objects.create(phone="+233242222222", role="PROVIDER")
        Provider.objects.create(
            user=user2, business_name="Low Rating", rating_avg=Decimal("3.5"), is_active=True
        )
        ...
        assert float(response.data["data"][0]["provider"]["rating_avg"]) >= float(
            response.data["data"][1]["provider"]["rating_avg"]
        )
...
class TestUpdateProviderEndpoint:
    ...
    def test_update_provider_wrong_owner(self, provider_client):
        ...
        other_user = User.objects.create(phone="+233249999999", role="PROVIDER")
        other_provider = Provider.objects.create(user=other_user, business_name="Other Provider")
...
class TestAddServiceEndpoint:
    ...
    def test_add_service_success(self, provider_client, provider_with_profile, plumbing_category):
        ...
        url = reverse(
            "providers:provider-add-service", kwargs={"pk": str(provider_with_profile.id)}
        )
        data = {
    ...
    def test_add_service_unauthenticated(
        self, api_client, provider_with_profile, plumbing_category
    ):
    ...
    def test_add_service_wrong_owner(self, provider_client, plumbing_category):
        ...
        # Create a different provider user
        other_user = User.objects.create(phone="+233249999998", role="PROVIDER")
        other_provider = Provider.objects.create(user=other_user, business_name="Other Provider")
...
class TestListProviderServicesEndpoint:
    ...
    def test_list_services_active_only(
        self, authenticated_client, provider_with_profile, plumbing_category
    ):
...
class TestUpdateServiceEndpoint:
    ...
    def test_update_service_success(
        self, provider_client, provider_with_profile, plumbing_category
    ):
    ...
    def test_update_service_wrong_owner(self, provider_client, plumbing_category):
        ...
        # Create a different provider user
        other_user = User.objects.create(phone="+233249999997", role="PROVIDER")
        other_provider = Provider.objects.create(user=other_user, business_name="Other Provider")
