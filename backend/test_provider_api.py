...
def test_provider_endpoints():
    ...
    try:
        ...
        provider_user = User.objects.create_user(
            phone="+233501234567", name="Test Provider", role="PROVIDER"
        )
        print(f"   [OK] Created provider user: {provider_user.phone}")

        # Create customer user
        customer_user = User.objects.create_user(
            phone="+233501234568", name="Test Customer", role="CUSTOMER"
        )
    ...
    response = client.get("/api/v1/providers/?latitude=5.6037&longitude=-0.1870&radius_km=10")
    print(f"   Status: {response.status_code}")
    if response.status_code == 200:
        data = response.json()
    ...
    update_data = {"business_name": "Updated Plumbing Services"}
    ...
    service_update = {"price_amount": "60.00"}
    response = client.patch(
