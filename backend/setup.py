...
def main():
    ...
    if not run_command("pip install -r requirements/development.txt", "Installing dependencies"):
        sys.exit(1)
    ...
    if not os.path.exists(".env"):
        ...
        print("=" * 60)
        subprocess.run(
            "copy .env.example .env" if os.name == "nt" else "cp .env.example .env", shell=True
        )
