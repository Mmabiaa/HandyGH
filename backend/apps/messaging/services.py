...
class MessagingService:
    ...
    @staticmethod
    def send_message(
        booking_id: str, sender: User, content: str, attachments: List[str] = None
    ) -> Message:
        """
        ...
        # Get booking
        try:
            booking = Booking.objects.select_related("customer", "provider", "provider__user").get(
                id=booking_id
            )
        ...
        message = Message.objects.create(
            booking=booking, sender=sender, content=content.strip(), attachments=attachments or []
        )

        logger.info(
            f"Message sent: {message.id} in booking {booking.booking_ref} " f"by user {sender.id}"
        )
    ...
    def get_booking_messages(
        booking_id: str, user: User, limit: int = None, offset: int = None
    ) -> QuerySet[Message]:
        """
        ...
        # Get messages
        messages = (
            Message.objects.filter(booking=booking).select_related("sender").order_by("created_at")
        )
    ...
    def _validate_booking_participant(booking: Booking, user: User) -> None:
        ...
        if not (is_customer or is_provider or is_admin):
            raise PermissionDenied("You do not have permission to access messages for this booking")
    ...
    def _validate_attachments(attachments: List[str]) -> None:
        ...
        for url in attachments:
            ...
            if not url.startswith(("http://", "https://")):
                raise ValidationError(f"Attachment URL must use HTTP or HTTPS protocol: {url}")
