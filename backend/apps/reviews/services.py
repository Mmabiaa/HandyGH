...
class ReviewService:
    ...
    @staticmethod
    def create_review(booking_id: str, customer: User, rating: int, comment: str = "") -> Review:
        """
    ...
    @staticmethod
    def get_provider_reviews(provider_id: str, limit: int = None) -> "QuerySet[Review]":
        """
        ...
        """
        queryset = (
            Review.objects.filter(provider_id=provider_id)
    ...
    def get_review_by_id(review_id: str) -> Review:
        ...
        return Review.objects.select_related(
            "customer", "provider", "provider__user", "booking"
        ).get(id=review_id)
...
class RatingAggregationService:
    ...
    def update_provider_rating(provider_id: str) -> Dict[str, Any]:
        ...
        aggregation = Review.objects.filter(provider_id=provider_id).aggregate(
        ...
        logger.info(
            f"Updated provider {provider_id} rating: " f"{avg_rating} ({total_reviews} reviews)"
        )

        return {"rating_avg": float(avg_rating), "rating_count": total_reviews}
