...
class CreateReviewSerializer(serializers.Serializer):
    ...
    rating = serializers.IntegerField(
        min_value=1, max_value=5, required=True, help_text="Rating from 1 to 5 stars"
    )
...
class ProviderRatingStatsSerializer(serializers.Serializer):
    ...
    rating_avg = serializers.FloatField(help_text="Average rating (0.00 to 5.00)")

    rating_count = serializers.IntegerField(help_text="Total number of reviews")

    rating_distribution = serializers.DictField(
        child=serializers.IntegerField(), help_text="Count of reviews per rating (1-5)"
    )
