...
class ProviderServiceCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_price_type(self, value):
        ...
        if value not in ["HOURLY", "FIXED"]:
            raise serializers.ValidationError("Price type must be either 'HOURLY' or 'FIXED'")
        return value
    ...
    def validate_duration_estimate_min(self, value):
        ...
        if value is not None:
            if value < 15:
                raise serializers.ValidationError("Duration estimate must be at least 15 minutes")
