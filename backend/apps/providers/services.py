...
class ProviderService:
        ...
        **kwargs,
        ...
        if categories:
            valid_categories = ServiceCategory.objects.filter(
                slug__in=categories, is_active=True
            ).values_list("slug", flat=True)

            invalid_categories = set(categories) - set(valid_categories)
            if invalid_categories:
                raise CustomValidationError(f"Invalid categories: {', '.join(invalid_categories)}")
    ...
    @staticmethod
    def verify_provider(provider_id: str, verification_doc_url: str = "") -> Provider:
        """
        Verify a provider.
...
class ServiceManagementService:
    ...
    def get_service(service_id: str) -> ProviderServiceModel:
        ...
        """
        try:
            return ProviderServiceModel.objects.select_related("provider", "category").get(
                id=service_id
            )
    ...
    def add_service(
        ...
        **kwargs,
        ...
        if price_type not in ["HOURLY", "FIXED"]:
            raise CustomValidationError("Price type must be either 'HOURLY' or 'FIXED'")
    ...
    def get_provider_services(
        provider_id: str, active_only: bool = True
        ...
        """
        queryset = ProviderServiceModel.objects.filter(provider_id=provider_id).select_related(
            "provider", "category"
        )
...
class ProviderSearchService:
    ...
    def search(
        ...
        if sort_by == "rating":
            ...
            results.sort(
                key=lambda x: (-float(x["provider"].rating_avg), -x["provider"].rating_count)
            )
