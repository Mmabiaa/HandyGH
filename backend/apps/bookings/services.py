...
class AvailabilityService:
    ...
    def check_availability(
        ...
        overlapping_query = Q(
            provider_id=provider_id, status__in=["REQUESTED", "CONFIRMED", "IN_PROGRESS"]
        ) & (
    ...
    @staticmethod
    @transaction.atomic
    def check_and_reserve(provider_id: str, start_time: datetime, end_time: datetime) -> bool:
        """
        ...
        conflicting_bookings = list(Booking.objects.filter(overlapping_query).select_for_update())
...
class BookingService:
    ...
    def calculate_booking_amount(
        provider_service: ProviderService, duration_hours: float
    ) -> Decimal:
        """
    ...
    def create_booking(
        ...
        if not is_available:
            raise ValidationError("Provider is not available for the selected time slot")

        # Calculate amount
        total_amount = BookingService.calculate_booking_amount(
            provider_service=provider_service, duration_hours=duration_hours
        )
...
class BookingStateMachine:
    ...
    def transition_status(
        booking: Booking, new_status: str, changed_by: User, reason: str = ""
    ) -> Booking:
        """
        ...
        if not BookingStateMachine.can_transition(booking.status, new_status):
            raise ValidationError(f"Cannot transition from {booking.status} to {new_status}")
    ...
    @staticmethod
    def decline_booking(booking: Booking, provider_user: User, reason: str = "") -> Booking:
        """
    ...
    def start_booking(booking: Booking, user: User) -> Booking:
        ...
        """
        return BookingStateMachine.transition_status(
            booking=booking, new_status="IN_PROGRESS", changed_by=user, reason="Service started"
        )
    ...
    def complete_booking(booking: Booking, user: User) -> Booking:
        ...
        """
        return BookingStateMachine.transition_status(
            booking=booking, new_status="COMPLETED", changed_by=user, reason="Service completed"
        )
    ...
    @staticmethod
    def dispute_booking(booking: Booking, user: User, reason: str = "") -> Booking:
        """
