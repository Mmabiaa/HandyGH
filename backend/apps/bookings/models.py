...
class Booking(models.Model):
    ...
    booking_ref = models.CharField(
        max_length=50,
        unique=True,
        help_text='Unique customer-facing booking reference'
    )
    ...
    scheduled_start = models.DateTimeField(
        help_text='When service is scheduled to start'
    )
    
    scheduled_end = models.DateTimeField(
        null=True,
        blank=True,
        help_text='When service is scheduled to end'
    )
    
    address = models.TextField(
        help_text='Location where service will be performed'
    )
