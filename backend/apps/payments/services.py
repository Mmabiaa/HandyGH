...
class CommissionService:
    ...
    def get_commission_rate(
        booking: Booking, provider_id: Optional[uuid.UUID] = None, category: Optional[str] = None
    ) -> Decimal:
        """
        ...
        if provider_id:
            provider_commission = Commission.objects.filter(
                provider_id=provider_id, applies_to="PROVIDER", is_active=True
            ).first()
        ...
        if category:
            category_commission = Commission.objects.filter(
                category=category, applies_to="CATEGORY", is_active=True
            ).first()
        ...
        default_commission = Commission.objects.filter(applies_to="ALL", is_active=True).first()
...
class PaymentService:
    ...
    def initiate_payment(
        booking: Booking, payment_provider: str = "MOMO", metadata: Optional[Dict[str, Any]] = None
    ) -> Transaction:
        """
        ...
        existing_txn = Transaction.objects.filter(
            booking=booking, status__in=["PENDING", "PROCESSING"]
        ).first()
        ...
        commission = CommissionService.calculate_commission(
            amount=booking.total_amount, booking=booking, provider_id=booking.provider.id
        )
        ...
        logger.info(f"Initiated payment transaction {txn.id} for booking {booking.booking_ref}")
    ...
    def process_payment_success(
        transaction_id: uuid.UUID, provider_ref: str, metadata: Optional[Dict[str, Any]] = None
    ) -> Transaction:
        """
        ...
        logger.info(
            f"Payment successful for transaction {txn.id}, " f"booking {booking.booking_ref}"
        )
    ...
    def process_payment_failure(
        transaction_id: uuid.UUID, reason: str, metadata: Optional[Dict[str, Any]] = None
    ) -> Transaction:
        """
        ...
        logger.warning(
            f"Payment failed for transaction {txn.id}, " f"booking {booking.booking_ref}: {reason}"
        )
    ...
    def _make_json_serializable(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            if isinstance(value, Decimal):
                ...
                result[key] = [
                    (
                        PaymentService._make_json_serializable(item)
...
class MoMoService:
    ...
    def _mock_charge(
        self, phone: str, amount: Decimal, reference: str, description: str
    ) -> Dict[str, Any]:
        """
        ...
        if success:
            ...
            logger.info(f"Mock MoMo charge successful: {phone}, {amount}, ref: {provider_ref}")
            return {
                "success": True,
