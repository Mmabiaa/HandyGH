...
class TransactionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Transaction Information', {
            'fields': ('id', 'booking', 'customer', 'provider')
        }),
        ('Payment Details', {
            'fields': ('amount', 'commission_amount', 'currency', 'status')
        }),
...
class CommissionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Commission Configuration', {
            'fields': ('id', 'name', 'rate', 'flat_fee', 'is_active')
        }),
        ('Application Rules', {
            'fields': ('applies_to', 'category', 'provider')
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
