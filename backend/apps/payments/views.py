...
from .services import MoMoService, PaymentService
...
class TransactionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == "ADMIN":
            return Transaction.objects.all().select_related("booking", "customer", "provider")
        elif user.role == "PROVIDER":
            # Provider sees transactions where they are the provider
            return Transaction.objects.filter(provider=user).select_related(
                "booking", "customer", "provider"
            )
        else:
            # Customer sees their own transactions
            return Transaction.objects.filter(customer=user).select_related(
                "booking", "customer", "provider"
            )
...
@swagger_auto_schema(
    ...
    responses={200: MoMoChargeResponseSerializer(), 400: "Bad request", 404: "Booking not found"},
)
@api_view(["POST"])
@permission_classes([IsAuthenticated])
def momo_charge(request):
    ...
    try:
        ...
        if booking.customer != request.user:
            return Response(
                {"success": False, "errors": {"message": "You can only pay for your own bookings"}},
                status=status.HTTP_403_FORBIDDEN,
            )

        # Initiate payment transaction
        txn = PaymentService.initiate_payment(
            booking=booking, payment_provider="MOMO", metadata={"phone": phone}
        )
        ...
        if charge_result["success"]:
            ...
            return Response(
                {"success": True, "data": response_serializer.data}, status=status.HTTP_200_OK
            )
        else:
            # Update transaction as failed
            txn = PaymentService.process_payment_failure(
                transaction_id=txn.id, reason=charge_result["message"], metadata=charge_result
            )
        ...
        return Response(
            {"success": False, "errors": {"message": "Booking not found"}},
            status=status.HTTP_404_NOT_FOUND,
        )
    except Exception as e:
        logger.error(f"Error processing MoMo charge: {str(e)}", exc_info=True)
        return Response(
            {"success": False, "errors": {"message": str(e)}},
...
@swagger_auto_schema(
    ...
    responses={200: "Webhook processed successfully", 400: "Invalid webhook data"},
)
@api_view(["POST"])
...
def momo_webhook(request):
    ...
    try:
        ...
        if existing_txn:
            ...
            return Response(
                {"success": True, "message": "Webhook already processed"}, status=status.HTTP_200_OK
            )
        ...
        try:
            ...
            return Response(
                {"success": False, "errors": {"message": "Transaction not found"}},
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        if webhook_status == "SUCCESS":
            PaymentService.process_payment_success(
                transaction_id=txn.id, provider_ref=provider_ref, metadata=serializer.validated_data
            )
        ...
        return Response(
            {"success": False, "errors": {"message": "Error processing webhook"}},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
@swagger_auto_schema(
    ...
    responses={200: TransactionSerializer(), 400: "Bad request", 404: "Booking not found"},
)
@api_view(["POST"])
@permission_classes([IsAuthenticated])
def manual_payment_confirm(request):
    ...
    try:
        ...
        logger.info(
            f"Manual payment submitted for booking {booking.booking_ref}: " f"ref={transaction_ref}"
        )
