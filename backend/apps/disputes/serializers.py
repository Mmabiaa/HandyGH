...
class DisputeListSerializer(serializers.ModelSerializer):
    ...
    raised_by_phone = serializers.CharField(
        source='raised_by.phone',
        read_only=True
    )
...
class DisputeUpdateSerializer(serializers.Serializer):
    ...
    status = serializers.ChoiceField(
        choices=Dispute.STATUS_CHOICES,
        required=True,
...
class DisputeResolveSerializer(serializers.Serializer):
    ...
    resolution = serializers.CharField(
        required=True,
        min_length=20,
        help_text='Resolution text explaining the decision'
    )
...
class AddEvidenceSerializer(serializers.Serializer):
    ...
    url = serializers.URLField(
        required=True,
        help_text='URL to the evidence file'
    )
    type = serializers.CharField(
        required=False,
        max_length=50,
        help_text='Type of evidence (photo, document, etc.)'
    )
    description = serializers.CharField(
        required=False,
        allow_blank=True,
