...
class Dispute(models.Model):
    ...
    reason = models.CharField(
        max_length=255, validators=[MinLengthValidator(5)], help_text="Short reason for the dispute"
    )

    description = models.TextField(
        validators=[MinLengthValidator(20)], help_text="Detailed description of the issue"
    )

    evidence = models.JSONField(
        default=list, blank=True, help_text="Evidence URLs and metadata (photos, documents, etc.)"
    )

    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default="OPEN", help_text="Current dispute status"
    )
