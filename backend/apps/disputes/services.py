...
class DisputeService:
    ...
    def get_dispute(dispute_id: str, user) -> Dispute:
        ...
        if not (
            user.is_admin or
            user.id == booking.customer.id or
            user.id == booking.provider.user.id
        ):
            raise PermissionDeniedError(
                "You don't have permission to view this dispute"
            )
...
class DisputeResolutionService:
    ...
    def resolve_dispute(
        ...
        if not resolution or len(resolution.strip()) < 20:
            raise ValidationError(
                "Resolution must be at least 20 characters long"
            )
    ...
    def close_dispute(dispute_id: str, admin_user) -> Dispute:
        ...
        if not admin_user.is_admin:
            raise PermissionDeniedError(
                "Only administrators can close disputes"
            )
